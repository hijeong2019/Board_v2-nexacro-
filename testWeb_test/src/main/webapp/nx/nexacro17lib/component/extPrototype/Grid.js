...
//grid propertiy -> 그리드에 griduserproperty 프로퍼티 생성해야 함
...
// 	var objGds = objApp.gdsUserGridCfgInfo;
// 	
// 	var sFormatId = this._getUniqueId(objGrid);
// 	var sFormat;
// 	var nFindRow = objGds.findRow("gridCfgId", sFormatId);
// 	if( nFindRow > -1){
// 		objGrid.orgformat2 = objGds.getColumn(nFindRow, "gridOriFmtXml");
// 		
// 		sFormat = "<Formats>" + objGds.getColumn(nFindRow, "gridFmtXml") + "</Formats>";
...
// 	}
	...
	this.gfnSetGridPersonal(objGrid);
...
pForm.gfnSetGridPersonal = function(objGrid)
{
	var objDs = objGrid.getBindDataset();
...
pForm._gfnGridSetCssclass = function (objGrid)
{
	...
	var clname = "exGrd_cell_WF_Fixed"; //yjlim.20200626.추가.
	clname = nexacro.wrapQuote(clname);
			
	objGrid.set_enableredraw(false);
	/* 원본 . 
	cssclass 를 교체하기 때문에
	이미 적용 된 cssclass가 적용이 안되서 주석처리 함.
	expr 사용하는 cssclass 건일 경우 cssclass 가 원복하기 불가능함.
...
// 	// grid format 개인화 작업 2019.05.23
// 	for(var i=0; i<objParentForm.components.length; i++)
// 	{
// 		var obj = objParentForm.components[i];
// 		if ( obj instanceof nexacro.PopupMenu) 
// 		{
// 			trace(obj);
// 			return;		
// 		}
...
pForm._gfnGridPersonalizeExcute = function (objGrid)
{
	...
	if( nFindRow == -1 ){
		...
		objGds.setColumn(nRow, "gridCfgId", sFormatId);
		objGds.setColumn(nRow, "gridFmtXml", sFormat);
		objGds.setColumn(nRow, "gridOriFmtXml", sOrgFormats);
	}else{
		objGds.setColumn(nFindRow, "gridFmtXml", sFormat);
		//objGds.setColumn(nFindRow, "sOrgFormat", sOrgFormats);
	}
	
	
// 	objGds.filter("gridCfgId =='" + sFormatId +"'");
// 	trace(objGds.saveXML());
	
// 	var dsParGridCfg = new Dataset;
// 	dsParGridCfg.set_id("dsParGridCfg");
// 	//dsParGridCfg.set_name("dsParGridCfg");
// 	dsParGridCfg.addColumn("gridCfgId", "string" ,256);
// 	dsParGridCfg.addColumn("gridFmtXml", "string" ,2147483647);
// 	dsParGridCfg.addColumn("gridOriFmtXml", "string" ,2147483647);
// 	
// 	var row = dsParGridCfg.addRow();
// 	dsParGridCfg.setColumn(row, "gridCfgId", sFormatId);
// 	dsParGridCfg.setColumn(row, "gridFmtXml",sFormat);
// 	dsParGridCfg.setColumn(row, "gridOriFmtXml", sOrgFormats);
	
	//헤더에 정렬아이콘도 저장되어서 삭제 함.20190612.yjlim
	var regExp1 = /▲/g;
	var regExp2 = /▼/g;
	sFormat = sFormat.replace(regExp1,"").replace(regExp2,"");
	sOrgFormats = sOrgFormats.replace(regExp1,"").replace(regExp2,"");
	//trace("sFormat:" + sFormat.replace(regExp1,"").replace(regExp2,""));
	
	objApp.dsParGridCfg.deleteAll();
	var row = objApp.dsParGridCfg.addRow();
	objApp.dsParGridCfg.setColumn(row, "gridCfgId", sFormatId);
	objApp.dsParGridCfg.setColumn(row, "gridFmtXml",sFormat);
	objApp.dsParGridCfg.setColumn(row, "gridOriFmtXml", sOrgFormats);
	
	var strSvcId    = "gridPersonal";
	var strSvcUrl   = "cmmn/userInfo/gridCfgProc.do";
	var inData      = "dsParGridCfg=dsParGridCfg";
	var outData     = ""  ;
	var strArg      = "";
...
/****** 그리드 복사 붙여넣기 추가. yjlim start ******/
/**
 * @class   그리드에서 선택 한 cell 값을 클립보드로 복사.
 * yjlim.20190830.추가
 * 프로퍼티 사용 안 할 시
 * @param {Object} grid - 대상 Grid Component
 * @return {Object} - 찾기 성공.
 * @example
 *  this.grdGridClipboardCopy(grid);
 ...
 *		if(keycode == 67){
 *			//copy
 *			this.grdGridClipboardCopy(obj)
 *		}else if(keycode == 86){
 *			//paste
 *			this.grdGridClipboardPaste();
 *		}
 *	}
 */
pForm.grdGridClipboardCopy = function (objGrid)
{
	var strSeparator = "\t";
	var orgDataset = eval("this."+objGrid.binddataset);
 ...
 * yjlim.20190830
 * 프로퍼티 사용 안 할 시
 * @return {String} - 클립모드 String.
 * @example
 *  this.grdGridClipboardPaste();
 */
pForm.grdGridClipboardPaste = function ()
{
	var strSeparator = "\t";
	// 숫자 자릿수 구분은 comma 를 사용하기 때문에 호환을 위해서 comma 를 제거한다.
	var strClipboardData = system.getClipboard("CF_TEXT").toString().split(",").join("");
	trace(" >>>>>>> [" + strClipboardData +"]");
	//var strClipboardData = this.arrClipboard.split(",").join("");
	...
	return strClipboardRecord;
};
/****** 그리드 복사 붙여넣기 추가. yjlim end ******/
