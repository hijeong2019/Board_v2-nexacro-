...
pForm.gfnExcelExport = function(objGrid,  sSheetName, sFileName)
{
	...
	//fileName nullcheck
	sFileName = this.gfnIsNull(sFileName) ? this.gfnGetArgument("menuNm")+"_"+this.gfnGetDate("time") : sFileName;
	//sheetName nullcheck
	...
	//둘중에 하나 사용.yjlim.20190625
	//this.objExport.set_exportuitype("none");
	this.objExport.set_exportuitype("exportprogress");
	
	//추가.yjlim.20190625
	this.objExport.set_exporttype(nexacro.ExportTypes.EXCEL2007); // Excel 2007
	this.objExport.set_transferrowcount(0);  //--> transferrowcount 추가 부분.서버에 넘기는 row 갯수
...
/**
 * @class excel export <br>
 * @param {Grid Array} objGrid - Grid Array
 * @param {String} [sFileName]	- file name
 * @return N/A
 * @example
 * this.gfnExcelExport([this.grid_export],"fileName");
 */
pForm.gfnMultiExcelExport = function(objArr,  sFileName)
{
	if( typeof objArr != "object" || objArr.length == 0 ){
		this.gfnExportOnerror();
	}
...
{	
	...
	// Excel Download Log by Daniel 20190709
	this.fnExDownLog();
...
// Excel Download Log by Daniel 20190709
pForm.fnExDownLog = function()
{
	var strSvcId    = "fnExDownLog";
	var strSvcUrl   = "/cmmn/reqLog/exDownLog.do";
	var inData      = "";
	var outData     = "";
	var strArg      = "";
 ...
 * CALLBACK 콜백 처리부분
 ************************************************************************************************/
pForm.fnCallback = function(svcID, errorCode, errorMsg)
{
	this.setWaitCursor(false);
