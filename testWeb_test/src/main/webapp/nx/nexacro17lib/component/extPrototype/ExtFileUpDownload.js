	...
	*/
	if (!nexacro.ExtFileUpload)
	{
		...
		*/
		...
		{
			trace("<<<<<<<<<<<<<<< _pExtFileUpload.on_fire_onerro >>  " + errorcode);
			trace("<<<<<<<<<<<<<<< _pExtFileUpload.on_fire_onerro >>  " + errormsg);
		...
		_pExtFileUpload.on_notify_filebutton_onclick = function (obj, e)
		{
			if (this._inputfiles && this.visible && this._isEnable() && this.enableevent)
			{
				...
				catch (e)
				{
					...
					this.on_fire_onerror(this, e.number, e.message, obj);
				}
			}
			return false;
		};
	  ...
	  _pExtFileUpload.on_fire_onloadstart = function (obj, type, evt, fileId)
	  {
		...
		{
			trace("<<<<<<<<<<<<<<< _pExtFileUpload.on_fire_onload  ");
				...
				if (this.readyState == 4 && this.status == 200) 
				{
					...
					else if (itemType == "download")
					{
						...
						else
						{
							...
							//pThis.on_fire_onerror(pThis, e.number, e.message, this, pFileId, pLoadItem.type);
	...
	*/
	if (!nexacro.ExtFileDownload)
	{
		...
		_pExtFileDownload.on_fire_onload = function (type, code, msg, url, fileId)
		{
			...
			} else if (this.onsuccess && this.onsuccess._has_handlers && url != "about:blank")
			{				
