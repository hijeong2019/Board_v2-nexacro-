...
*  2017.11.14     	consulting  	            Validation 등록 방식을 User property에서 
                                                script 방식으로 변경
*******************************************************************************
*/
...
pForm.gfnValidationCheckRule = function(itemName, itemValue, validationRuleSet, compare1, compare2)
{
	...
	switch(checkrule)
	{
		...
		// 숫자 여부 : digits
		case "digits":
			if (!this.lookupFunc("gfnIsDigit").call(itemValue)) //20200206.yjlim.원본  //콤마제거 함수추가도 있지만 data를 변경해서 검증처리해야 함.
			{
		...
		// 날짜 년월일 체크 : date
		case "date":
			// gfnIsYMD -> gfnIsDate 으로 변경.yjlim.20190829
			if (!this.lookupFunc("gfnIsDate").call(itemValue))
			{
		...
		// 사이의 값인지 비교 - range:40:100
		case "rangefloat":
			if (parseFloat(itemValue) < parseFloat(arrItem2[1]) || parseFloat(itemValue) > parseFloat(arrItem2[2]))
			{
				//return titile + "의 값은 "+arrItem2[1]+" 와(과) "+arrItem2[2]+" 사이의 값이어야 합니다.";
				// {0} 은(는) {1} 와(과) {2} 사이의 값입니다.
				return this.gfnGetMessage("msg.err.validator.range",[titile,arrItem2[1],arrItem2[2]]);
