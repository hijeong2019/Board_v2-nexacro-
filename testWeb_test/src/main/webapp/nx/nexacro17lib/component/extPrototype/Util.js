...
************************************************************************************************/
/**
 * @class 글로벌 데이터셋의 사용자 정보를 gbn 값에 맞게 가져온다.
 * by yjlim
 */
pForm.gfnGetUserInfo = function(gbn)
{
	if( this.gfnIsNull(gbn) ) gbn = "userEmpNo";
	
	var rVal = "";
	rVal = this.gfnGetApplication().gdsUserInfo.getColumn(0,gbn);
	return rVal;
};

/**
 * @class 글로벌 권한 DS에서 해당 권한이 있는지 체크
 * @param {String} sValue	
 * @return {Boolean} true/false
*/
pForm.gfnIsAuthInfo = function(arrArgs)
{
	//배열 여부 체크.
	if( this.gfnIsNull(arrArgs) || !this.gfn_isArray(arrArgs) ) {
		return false;
	}
	
	for(var idx=0; idx < arrArgs.length; idx++){
		var checkVal = arrArgs[idx];
		
		var sRow = this.gfnGetApplication().gdsUserAuthInfo.findRow("authId",checkVal);
		if( sRow != -1 ){
			return true;
		}
	}
...
/**
 * @class 입력 문자열중 숫자값만 남긴다. <br>
 * 문자 체크 후 기본 숫자를 0으로 리턴한다. <br>
 * @param {String} strValue - 입력문자열
 * @return {String} 숫자문자열
 * YJLIM. 20200330. 추가.
 */
pForm.gfnGetOnlyNumber = function(val)
{
...
pForm.gfnIsDate = function(strDate)
{
	...
	switch(strDate.length)
	{
		...
		case 8://년월일
			...
			break;
		case 10://년-월-일
			retVal =  this.gfnIsFmtYMD(strDate);
			break;
		default:
			retVal = false; 
			break;
...
/**
 * @class 날짜 여부를 확인한다. <br>
 * @param {String} strDate - 10자리의 숫자로 된 날짜(YYYY-MM-DD) 또는 YYYY.MM.DD
 * @return {Boolean}
 * 추가.yjlim.20190829
 */
pForm.gfnIsFmtYMD = function(strDate)
{
	var retVal = this.gfnGetDigit(strDate); //숫자로 return YYYYMMDD
	
	if (retVal.length != 8) {
		return false;
	}

	var strYM = retVal.substr(0,6);	//년월
	if (!this.gfnIsYM(strYM)) {
		return false;
	}
	var nDay   = Number(retVal.substr(6,2));	// 일자
	var nLastDay = Number(this.gfnGetLastDate(strYM).substr(6,2));//gfnGetLastDay에서 전체 20170331값이 넘어와서 .substr(6,2)추가 20170313
	if (nDay < 1 || nDay > nLastDay) {
		return false;
	}
...
/**
 * @class YYYYMMDD을 입력받아 대쉬형태의 날짜를 반환한다. <br>
 * @param {String} strDate - 8 자리의 숫자로 된 날짜(YYYYMMDD)
 * @return {String} YYYY-MM-DD
 */
pForm.gfnDashDate = function(strDate)
{
	var s = "";
    if (strDate == null || strDate == "" || strDate.length != 8) {
		...
		s = strDate.substr(0,4) + "-" + strDate.substr(4,2) + "-" + strDate.substr(6,2)
    }
